...
#include "CESPDIN.H"
#include "GENERICO.h"
#include "IdTiposEspaco.def"     //OBS: Usar esse no VISUAL STUDIO (Colocar arquivo na pasta dos .c e .h
//#include   "..\\tabelas\\IdTiposEspaco.def"  OBS: Usar esse SEM ser pelo Visual Studio
#endif
...
static GRA_tpCondRet GRA_EsvaziarGrafo(GRA_tppGrafo pGrafo );
...
*  ****/
   GRA_tpCondRet GRA_ExcluiVerticeCorrente( GRA_tppGrafo pGrafo )
   {
	   ...
	   if(pGrafo->pVerticeCorrente->arestas != NULL){
		   ...
		   qtd = LIS_ContaElementos(pUser->arestas);
		   LIS_IrInicioLista(pUser->arestas);
		   for(i=0;i<qtd;i++){
			   #ifdef _DEBUG
				 CNT_CONTAR( "GRA_ExcluiVerticeCorrente-for0" ) ;
			   #endif
			   idAmigo = (char*) LIS_ObterValor(pUser->arestas);
			   condGra = GRA_VerificaVerticeExiste(pGrafo, idAmigo);  
			   if(condGra == GRA_CondRetOK){
				   GRA_ExcluirAresta(pGrafo, pUser->id, idAmigo);
			   } 
  ...
  GRA_tpCondRet GRA_VerificaVerticeExiste( GRA_tppGrafo pGrafo, void *id)
   {
	   ...
	   while (LIS_ObterValor( pGrafo->pListaVertices ) != NULL )
	   {
		   ...
		   String1 = (char*)pVerticeGrafo->pValor;
	       if(pVerticeGrafo != NULL && strcmp(String1,String2) == 0){
			   #ifdef _DEBUG
